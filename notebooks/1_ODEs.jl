...
In this small tutorial, we will demonstrate how to solve ordinary differential equations (ODEs) in Julia. For detailed information and advanced use (solvers, stochastic, Gillespie equations), we refer to the official [documentation](https://diffeq.sciml.ai/stable/tutorials/ode_example/). In this tutorial, we will study some basic growth models for plants.
"""

# ╔═╡ e3aaa1fd-a4f9-4569-9c43-fb867583e812
md"""
## ODEs with a single variable

Often, we can describe our system with a single variable $x$. A first-order differential equation is described by:

$$\frac{\text{d}x}{\text{d}t} = f(x, \theta, t)\,,$$

where we have:
- our variable of interest $x(t)$, changing with time $t$;
- the derivative with respect to time $\frac{\text{d}x}{\text{d}t}$;
- any potential parameters $\theta$ that determine the behaviour.

To solve such an ODE, we require an *initial condition*, usually of the form $x(0) = x_0$.

As an example, we will study *logistic growth*. 

$$\frac{\text{d}x}{\text{d}t} = \mu x (1 - x / K)$$

Here $x(t)$ represents the *biomass* at time $t$, $\mu$ is the *growth rate* and $K$ the *carrying capacity*. We can easily implement this as a function.
"""
...
md"Set some default values for $\mu$ and $K$."
...
md"We then need to specify the initial value"
...
md"""
## Fitting the parameters of an ODE

We have some data of dry matter accumulation of perennial ryegrass at Hurley, England. The data is stored as pairs of (time [weeks], dry matter [Mg ha^-1]"). 
"""
...
md"Unpack the data so we can plot it"

# ╔═╡ e191d93e-54bd-4c78-8292-bfce18fb0f68
ts, xs = first.(data), last.(data)

# ╔═╡ 4f8e1ca6-02b5-4b96-889c-25583260ddd0
scatter(ts, xs, xlabel="t [weeks]", label="Dry matter [Mg ha^-1]", legend=:bottomright)
...
function sse(μ, K)
	prob = ODEProblem(f, 1.0, (14.4, 30.), (μ, K))
	sol = solve(prob, saveat=ts)  # solve at time measurements
	xsim = sol.u
...
optsol = optimize(((μ, K),)->sse(μ, K), [0.1, 5.0])

# ╔═╡ 2f5bdbaf-41fe-401c-a7d6-ae345f4fd10a
optsol.minimizer  # best μ and K

# ╔═╡ aed49ea7-85bf-4201-8ebc-50e3e2e3ac2b
let
	prob = ODEProblem(f, 1.0, (14.4, 30.), optsol.minimizer)
	sol = solve(prob)  # solve at time measurements
	xsim = sol.u
	plot(sol, label="fit")
	scatter!(ts, xs, xlabel="t [weeks]", label="Dry matter [Mg ha^-1]", legend=:bottomright)
end
...
Rstar([10, 20, 2], (1.0, 0.1, 2., 0.1), 0)  # example
...
# ╔═╡ 4dd6f0a8-6eeb-4afe-96ee-b0c64f7a05b7
a1, a2 = 0.006, 0.012

# ╔═╡ 13736731-03ef-48ee-80ee-b117f84b9615
r = 1.0

# ╔═╡ b9b87dce-f8a7-4248-ac9c-42bdd235b5cf
prob2 = ODEProblem(Rstar, u0, (0.0, 50.0), (a1, a2, d, r))

# ╔═╡ 60116697-b961-4590-992a-ae03e1449c26
sol2 = solve(prob2)

# ╔═╡ 41e211c5-b224-49dd-91c7-57870c807464
plot(sol2, labels=["N₁" "N₂" "R"], lw=2)
...
git-tree-sha1 = "61d8e295c33ed44dca9ca37fd7e825fd7a5a43cd"
