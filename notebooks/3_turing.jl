...
# ╔═╡ a0b830ea-5dd5-4fab-ac0b-f632ef595523
using Turing, Plots, StatsPlots

# ╔═╡ 7f8ac937-31f4-4339-9c21-0061bdce022b
using PlutoUI

# ╔═╡ 41e1257d-7415-41a8-9abb-5189acdd7e8a
using DynamicHMC

# ╔═╡ 002f921e-2efb-11ed-1b6d-49c17faf8dd2
md"""
# Automatic inference for probabilistic models using Turing.jl

Turing.jl is a probabilistic programming language that is used to perform automatic Bayesian statistical inference using a variety of sampling-based algorithms. In this tutorial we will illustrate its use for A\B testing and for evaluating a few basic plant growth models.
"""

# ╔═╡ 42af88d3-8370-4ff1-b3f7-02dff8717eb9
md"""
## A/B testing to compare treatment effects

A/B testing or split-run testing is a standard research methodology in which a randomized experiment is conducted to compare the results of two variants (e.g. treatment versus control). An example of a setting where A/B testing could be applied in plant science is when the crop yield is compared for the application of two different pesticides A and B.



try making adjustments to the values of N_A_{+}, N_A_{-}, N_B_{+}, and N_B_{-} to see what the effect is on the outcomes and plots.
"""

# ╔═╡ 2cca6509-88b1-4a95-8e54-dd722b9b4916
...
# ╔═╡ baa6bdd4-c34f-448d-8e3a-6f5d89ef6a21
N_A₊ = 29

# ╔═╡ 37299d3d-e1a7-4fdd-a440-08ed3ce4bbe0
N_A₋ = 1

# ╔═╡ 2084a969-c0cb-406b-b235-dc11c4cdf947
N_B₊ = 17

# ╔═╡ 02d26e0b-cda7-4a01-8566-dad649c5e422
N_B₋ = 3

# ╔═╡ 3bd82cbe-6d53-42d1-a1fe-565c6b5f97f8
md"""
Exact inference is often intractable using Bayesian statistics, therefore approximations such as Markov Chain Monte Carlo (MCMC) sampling algorithms are typically used. Turing.jl has implementations of several of these algorithms. The No-U-Turn Sampler (NUTS) used in the code below is an example thereof.
"""

# ╔═╡ 8218e425-7c57-44da-b0fa-f994155f72a7
# ╠═╡ show_logs = false
chain1 = sample(treatment(N_A₊, N_A₋, N_B₊, N_B₋), NUTS(), 1000, drop_warmup=true);

# ╔═╡ 90f1a1ac-dad8-4b9e-b72e-234e3ced1b8d
plot(chain1)

# ╔═╡ e656f5b8-58e6-46af-bf2b-3eafe9820c86
p_A_better_than_B = mean(chain1[:p_A] .> chain1[:p_B])

# ╔═╡ fb6716f3-1a75-47a1-be2b-93c937cd5f9d
p_A_much_better_than_B = mean(chain1[:p_A] .> 1.2chain1[:p_B])

# ╔═╡ b15da6e5-4e01-401a-b44f-fee5a92a7248
md"""
## Probabilistic curve fitting for growth models

In this section, two basic plant growth models are evaluated for an example dataset consisting of time points and their corresponding plant biomass.

"""

# ╔═╡ e803208d-7234-4e43-a344-0b72168420f4
data = [(14.4, 1.0), (15.9, 2.3), (17.3, 4.0), (18.7, 6.0), (20.1, 7.9), (21.6, 9.2), (23.1, 9.8), (24.6, 9.9)]

# ╔═╡ bafe8e7f-f6a5-4486-8c43-07ef00a49432
t, y = first.(data), last.(data)

# ╔═╡ f57c3b76-ad09-4d9e-98b7-4cb242d94809
md"""
### Gompertz growth model

The classic Gompertz growth model is a sigmoid function that describes growth as being the slowest at the beginning and end of a given period. It is mathematically formulated as:

$$f(t,a,b,c) = ae^{-be^{-ct}}$$

Here $a$ is the maximum biomass (i.e. the asymptote to which the biomass converges), $b$ is the horizontal displacement along the time axis, $c$ is the growth rate, and $e$ is Euler's number.
"""

# ╔═╡ ec58ab01-03b2-48ad-8a7c-713006cd3103
gompertz(t; a, b, c) = a * exp(-b * exp(-c * t))

# ╔═╡ 967c7c5d-257c-48a4-938c-bd271642f2f7
md"""
In the code below, exponential priors are chosen for the parameters $a$, $b$, and $c$. Next, our observed data is modeled as samples from a Normal distribution with the Gompertz function described above as its mean. A Gamma function prior is used to model the variance of the normal distribution.
"""

# ╔═╡ 0d3225a7-1b82-4703-a9ba-a3f2f93dc58c
...
# ╔═╡ 17a57284-1444-4833-ac7e-cc4840a8ace5
md"""
The next step is to sample from the posterior distribution using a variant of the Hamiltonian Monte Carlo method.
"""

# ╔═╡ 61c2b31b-518b-406b-b340-3900bb5e8c8f
chain2 = sample(gompertz_fit(t, y), NUTS(), 1000, drop_warmup=true)

# ╔═╡ c5b72c79-b58d-4fdf-be6f-a65207ce1191
plot(chain2)

# ╔═╡ 5903a957-7e50-4ec0-a231-4fd866c4e9a9
md"""
Once we have generated a large number of samples, we can use them to estimate the values of the Gompertz function's parameters based on the available data.
"""

# ╔═╡ df9b99f6-8aa8-4454-b592-5361a38d4442
â = mean(chain2[:a])

# ╔═╡ 20e93054-2fb7-48c6-b23b-038d0d9817e2
b̂ = mean(chain2[:b])

# ╔═╡ dbf00ddf-d0fa-4a46-9944-b3e4b5cee6a3
ĉ = mean(chain2[:c])

# ╔═╡ 7f219f68-5335-4628-9a01-873592f4f167
begin
	plot(t->gompertz(t, a=â, b=b̂, c=ĉ), 0, 26, label="MAP")
	scatter!(t, y, label="data")
end

# ╔═╡ 80ea1431-301e-4f0d-89e8-be1a2561dbcf
md"""
### Logistic growth model

The logistic function is an alternate function used to model growth that also gives rise to a sigmoid curve:

$$f(t,L,k) = \frac{L}{1 + e^{-k(t-t_{0})}}$$

here $L$ is the maximum biomass, $k$ is the logistic growth rate, and $t_{0}$ is the time elapsed when measurement starts.

"""

# ╔═╡ f75be78d-8248-4186-9cbb-a63cd4f35a83
logistic(t; t₀, L, k) = L / (1 + exp(-k * (t - t₀)))

# ╔═╡ bdf3d0e4-3671-4211-bf20-7b789f72c617
md"""
Uniform, Exponential and Lognormal prior distributions are chosen to model the parameters $t_{0}$, $L$, and $k$, respectively.

The Markov Chain Monte Carlo sampling to estimate the parameters of the logistic growth function is similar to what was done when evaluating the Gompertz growth curve.

"""

# ╔═╡ 6a641aa4-b873-4bba-b8d2-fbfa1057ac0f
@model function logistic_fit(t, y)
    t₀ ~ Uniform(0, 25)
    L ~ Exponential(10)
    k ~ LogNormal()
    σ² ~ Gamma(2)
...
# ╔═╡ 9201ce8a-10b8-44ec-a680-c14fc5753f2f
chain3 = sample(logistic_fit(t, y), NUTS(), 1000, drop_warmup=true)

# ╔═╡ dac16af9-9d3d-4ea9-ac74-59f41ae71cc3
t̂₀ = mean(chain3[:t₀])

# ╔═╡ e7f89da2-7ddf-40f1-b81b-f68602747293
L̂ = mean(chain3[:L])

# ╔═╡ 0e2364f5-1858-4490-aaa8-fedb1cda4498
k̂ = mean(chain3[:k])

# ╔═╡ 88ca6774-80e0-4144-8486-f993c7373322
plot(chain3)

# ╔═╡ d0415d1b-090f-43b3-861d-e53cd5e4f53b
begin
	p = plot(t->logistic(t; t₀=t̂₀, L=L̂, k=k̂), 0, 26, label="MAP")
	scatter!(p, t, y, label="data")
end

# ╔═╡ a5d8429d-5c8c-477a-b582-51860bbdb42b
for i in 1:10:1000
    plot!(p, t->logistic(t; t₀=chain3[:t₀][i], L=chain3[:L][i], k=chain3[:k][i]),
            0, 26, alpha=0.1, color="blue", label="")
end

# ╔═╡ 6bd49b4f-ba63-407c-9675-b6ad30ab7da8
p

# ╔═╡ 1ca16a42-9a35-461e-9fca-752f154085f1
scatter(chain3[:L][:], chain3[:k][:], alpha=0.6, ylabel="k", xlabel="L")

# ╔═╡ 1a0fe1c6-f3d8-4921-a95e-e258be7b674c
md"""

Exercise: evaluate how the choice of prior distributions in the models can affect the outcome.
